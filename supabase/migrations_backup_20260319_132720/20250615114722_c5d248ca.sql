-- Drop policies that depend on property_id before altering the column type
...
-- Drop FK first if it exists (may block type change)
ALTER TABLE public.maintenance_requests DROP CONSTRAINT IF EXISTS fk_property;
ALTER TABLE public.maintenance_requests
...
FOR INSERT
WITH CHECK (user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role::text = 'tenant'));
