...
-- User Preferences Policies (DROP IF EXISTS for idempotency)
...
-- Property Interactions Policies (DROP IF EXISTS for idempotency)
DROP POLICY IF EXISTS "Users can view their own interactions" ON property_interactions;
DROP POLICY IF EXISTS "Users can insert their own interactions" ON property_interactions;
DROP POLICY IF EXISTS "Property owners can view interactions on their properties" ON property_interactions;
...
-- Smart Recommendations Policies (DROP IF EXISTS for idempotency)
DROP POLICY IF EXISTS "Users can view their own recommendations" ON smart_recommendations;
DROP POLICY IF EXISTS "Users can update their own recommendations" ON smart_recommendations;
DROP POLICY IF EXISTS "System can insert recommendations" ON smart_recommendations;
...
-- Trigger for user_preferences updated_at (DROP IF EXISTS for idempotency)
