...
export const processScreening = async (screeningId: string) => {
  try {
    ...
    const mockResults = [
      { type: 'background', status: 'completed', verified: Math.random() > 0.2, score: Math.floor(Math.random() * (100 - 50 + 1)) + 50 },
      { type: 'credit', status: 'completed', verified: Math.random() > 0.1, score: Math.floor(Math.random() * (850 - 500 + 1)) + 500 },
    ...
    toast.success("Screening Completed", { description: "The tenant screening process is complete." });
    return true;
  } catch (error: any) {
