...
export const fetchFinanceDashboardData = async ({
  timeframe,
  propertyFilter,
}: FetchFinanceDashboardDataProps) => {
  ...
  const revenueByMonth = months.reduce(
    (acc, month) => ({ ...acc, [month]: 0 }),
    {} as Record<string, number>
  );
  const expensesByMonth = months.reduce(
    (acc, month) => ({ ...acc, [month]: 0 }),
    {} as Record<string, number>
  );
  ...
  if (payments) {
    payments.forEach((p) => {
      if (p.payment_date) {
        ...
        if (Object.prototype.hasOwnProperty.call(revenueByMonth, month)) {
  ...
  // --- Fetch Expense Data (public.expenses; legacy code referenced non-existent finance_transactions) ---
  ...
  if (expenses) {
    expenses.forEach((e) => {
      if (e.created_at) {
        ...
        const month = date.toLocaleString('default', { month: 'short' });
        if (Object.prototype.hasOwnProperty.call(expensesByMonth, month)) {
  ...
  if (propertyFilter !== 'all') {
    ...
    if (ptError) {
      ...
      const propertyTenantMap = allPropertyTenants.reduce(
        (acc, pt) => {
