...
export const getDocumentContent = (role: string, guide: string) => {
  ...
  const { title, description, topics } = getOverviewContent(roleDisplayName, guideDisplayName, guide);
...
const getGuideDisplayName = (guide: string): string => {
  switch (guide) {
    case 'complete': return 'Complete Guide';
...
const getOverviewContent = (roleDisplayName: string, guideDisplayName: string, guideType: string) => {
...
const getSectionsList = (role: string, guide: string) => {
  ...
  if (guide === 'complete') {
    ...
    sections = [...sections, ...(roleSections[role]?.slice(0, 1) || []), ...(guideSections[guide] || [])];
  }
