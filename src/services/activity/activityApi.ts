...
import { validatePaginationParams, formatDate, validateActivityData, handleServiceError } from './utils';

/**
 * Fetch activities with pagination
 */
export const fetchActivities = async (page = 1, pageSize = 10): Promise<{ data: ActivityItem[], count: number }> => {
  try {
 ...
 * Add a new activity
 */
export const addActivity = async (activity: Omit<ActivityItem, 'id' | 'date'>): Promise<boolean> => {
  try {
    // Validate input data
