// Voice Assistant Service for Nigeria Homes Platform
// Created: 2025-08-01
...
export class VoiceAssistantService {
  ...
  public async startListening(): Promise<void> {
    if (!this.recognition) {
      throw this.createError('SPEECH_RECOGNITION_NOT_SUPPORTED', 'Speech recognition not supported');
    }
  ...
  public async processVoiceCommand(transcript: string, confidence: number = 1.0): Promise<VoiceResponse> {
    try {
      ...
      if (this.settings.auto_execute_commands && response.action && !response.requires_confirmation) {
  ...
  private extractEntities(transcript: string, entityTypes: any[]): VoiceEntity[] {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (word.includes('naira') || word.includes('₦') || 
  ...
  private async generateResponse(command: VoiceCommand): Promise<VoiceResponse> {
    ...
    let responseText = this.fillTemplate(template, command.entities);
  ...
  private async executeAction(action: VoiceAction): Promise<void> {
    switch (action.type) {
      ...
      case 'open_modal':
        ...
        window.dispatchEvent(new CustomEvent('voice-open-modal', { 
      ...
      case 'start_process':
        ...
        window.dispatchEvent(new CustomEvent('voice-start-process', { 
          detail: action.payload 
        }));
        break;
  ...
  private generateSuggestions(intent: VoiceIntent): string[] {
    const suggestions: Record<VoiceIntent, string[]> = {
      ...
      'check_payments': ['Show payment history', 'Check outstanding payments', 'View payment analytics'],
      'add_property': ['Add new property', 'Create property listing', 'Register property'],
      'send_reminder': ['Send payment reminder', 'Send maintenance reminder', 'Send lease renewal reminder'],
  ...
  private getCommandPatterns(): VoiceCommandPattern[] {
    return [
      ...
      },
      {
        ...
        response_templates: ['I\'ll help you add a new property. Opening the property creation form.'],
      ...
      },
      {
        ...
        response_templates: ['I can help you manage properties, check payments, view tenants, and more. Try saying "show my properties" or "check payments".']
      }
    ];
  }
