...
export const MobileDemo = () => {
  ...
  const { currentIndex, goToNext, goToPrevious, swipeRef: carouselRef } = useSwipeableCarousel(
    sampleProperties.length,
    (index) => console.log('Property carousel index:', index)
  );

  // Swipeable tabs
  const { activeTab, goToTab, swipeRef: tabsRef } = useSwipeableTabs(
    4,
    0,
    (tabIndex) => {
  ...
  // Pull to refresh
  const { isPulling, isRefreshing, pullProgress, swipeRef: refreshRef } = usePullToRefresh(
    async () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <MobileNavBar
        title="Nigeria Homes Mobile Demo"
        showBack={true}
      ...
      {isPulling && (
        <div className="fixed top-16 left-0 right-0 z-50 bg-blue-500 text-white text-center py-2">
          <div className="flex items-center justify-center gap-2">
            <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent" />
            {isRefreshing ? 'Refreshing...' : `Pull to refresh (${Math.round(pullProgress * 100)}%)`}
          </div>
        </div>
      )}

      <div ref={refreshRef} className="pt-16 pb-20">
        {/* Device Info Header */}
        <div className="bg-white border-b p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {deviceInfo.isMobile ? <Smartphone className="h-6 w-6 text-blue-600" /> :
               deviceInfo.isTablet ? <Tablet className="h-6 w-6 text-green-600" /> :
               <Monitor className="h-6 w-6 text-gray-600" />}
              <div>
                <h1 className="text-lg font-semibold">Mobile Demo</h1>
                <p className="text-sm text-gray-600 capitalize">
                  {String(deviceInfo.deviceType)} • {deviceInfo.isTouchDevice ? 'Touch' : 'No Touch'} • 
              ...
              <TabsContent value="components" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    {/* Mobile Buttons */}
                    <div>
                      <h4 className="font-medium mb-3">Mobile Buttons</h4>
                      <div className="flex flex-wrap gap-3">
                        <MobileButton size="sm" variant="default">Small</MobileButton>
                        <MobileButton size="md" variant="outline">Medium</MobileButton>
                        <MobileButton size="lg" variant="secondary">Large</MobileButton>
                        <MobileButton size="xl" variant="destructive">Extra Large</MobileButton>
                      </div>
                    </div>

                    {/* Mobile Search */}
                    <div>
                      <h4 className="font-medium mb-3">Mobile Search</h4>
                      <MobileSearchBar
                    ...
                    {/* Mobile Form Inputs */}
                    <div>
                      <h4 className="font-medium mb-3">Mobile Form Inputs</h4>
              ...
              <TabsContent value="gestures" className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Swipe, tap, and long press interactions
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    {/* Swipe to Dismiss Notifications */}
                    <div>
                      <h4 className="font-medium mb-3">Swipe to Dismiss Notifications</h4>
                      <div className="space-y-2">
                        {notifications.map(notification => {
                          const { swipeRef } = useSwipeToDismiss(
                            (direction) => handleDismissNotification(notification.id, direction),
                            80
                          );
                    ...
                    {/* Long Press Demo */}
                    <div>
                      <h4 className="font-medium mb-3">Long Press Interaction</h4>
                      <div
                        ref={longPressRef}
                        className={`bg-purple-50 border-2 rounded-lg p-6 text-center cursor-pointer transition-colors ${
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        ...
                        The platform supports touch gestures, proper sizing, and Nigerian market optimizations.
                      </AlertDescription>
                    </Alert>
                  </CardContent>
                </Card>
