...
const TenantOnboardingPage = () => {
  ...
  const fetchTenants = async () => {
    ...
    try {
      ...
      const tenantsWithProperties = await Promise.all(tenantsData.map(async (tenant) => {
                          ...
                          {filteredTenants.map(tenant => (
                            ...
                            >
                              <div className="flex flex-col items-start">
                                ...
                                <span className="text-xs text-muted-foreground">{tenant.property_name || 'No property assigned'}</span>
                              </div>
                            </Button>
                          ))}
                        </div>
                ...
                {selectedTenant && selectedTenant.property_id ? (
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <p className="text-center text-muted-foreground">This tenant does not have a property assigned.</p>
                    </CardContent>
                  </Card>
                ) : (
                  <Card>
                    <CardContent className="pt-6">
                      <p className="text-center text-muted-foreground">Select a tenant to view their onboarding status.</p>
                    </CardContent>
                  </Card>
                )}
              </div>
