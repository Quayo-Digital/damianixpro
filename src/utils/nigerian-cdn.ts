...
export class NigerianCDN {
  ...
  private constructor() {
    this.config = {
      primaryNodes: ['https://cdn-lagos.nigeriahomes.com', 'https://cdn-abuja.nigeriahomes.com'],
      fallbackNodes: ['https://cdn-west-africa.cloudfront.net', 'https://cdn-africa.fastly.com'],
      geolocation: {
      ...
      async () => {
        try {
          ...
          metrics.forEach((metric) => {
            if (metric.latency > 2000) {
              // 2 seconds threshold for Nigerian networks
              console.warn(`High CDN latency detected in ${metric.location}: ${metric.latency}ms`);
            }

            if (metric.availability < 95) {
              console.error(
                `CDN availability issue in ${metric.location}: ${metric.availability}%`
              );
            }
          });
  ...
  getPerformanceAnalytics(): {
    ...
    const avgLatency =
      this.performanceMetrics.reduce((sum, m) => sum + m.latency, 0) /
      this.performanceMetrics.length;

    const locationStats = this.performanceMetrics.reduce(
      (acc, metric) => {
    ...
    const avgAvailability =
      this.performanceMetrics.reduce((sum, m) => sum + m.availability, 0) /
      this.performanceMetrics.length;
