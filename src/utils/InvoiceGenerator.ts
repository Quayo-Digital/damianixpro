...
export const generateInvoicePdf = (invoice: Invoice): string => {
  ...
  doc.text(`Date: ${new Date(invoice.date).toLocaleDateString()}`, pageWidth - 20, 35, { align: 'right' });
  doc.text(`Due Date: ${new Date(invoice.dueDate).toLocaleDateString()}`, pageWidth - 20, 40, { align: 'right' });
  ...
  doc.text('Please make payment through the tenant portal or using the payment options provided.', 20, finalY + 50);
