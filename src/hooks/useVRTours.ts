...
interface UseVRToursReturn {
  ...
  searchTours: (filter: VRTourFilter, sort: VRTourSort, page?: number) => Promise<VRTourSearchResult>;
  ...
  updateScene: (tourId: string, sceneId: string, updates: Partial<VRScene>) => Promise<VRScene | null>;
  ...
  updateHotspot: (tourId: string, hotspotId: string, updates: Partial<VRHotspot>) => Promise<VRHotspot | null>;
...
export function useVRTours(options: UseVRToursOptions = {}): UseVRToursReturn {
  const { userId, autoStart = false, enableAnalytics = true, enablePerformanceMonitoring = true } = options;
  ...
  const updateSceneMutation = useMutation({
    mutationFn: async ({ tourId, sceneId, updates }: { tourId: string; sceneId: string; updates: Partial<VRScene> }) => {
  ...
  const addHotspotMutation = useMutation({
    mutationFn: async ({ tourId, hotspotData }: { tourId: string; hotspotData: Partial<VRHotspot> }) => {
  ...
  // Session Management
  const startTourSession = useCallback(async (tourId: string): Promise<VRTourSession> => {
    if (!hasVRAccess) {
  ...
  // Event Tracking
  const trackEvent = useCallback((event: Omit<VRTourEvent, 'timestamp' | 'sessionId'>) => {
  ...
  // Analytics
  const getAnalytics = useCallback(async (tourId: string): Promise<TourAnalytics | null> => {
  ...
  const exportTour = useCallback(async (tourId: string): Promise<Blob> => {
  ...
  const importTour = useCallback(async (file: File): Promise<VRTour> => {
    if (!canCreateTours) {
