...
export function useShortletCalendar(): UseShortletCalendarReturn {
  ...
  const refreshCalendar = useCallback(
    async (listingId: string, startDate: string, endDate: string) => {
      setIsLoading(true);
      try {
  ...
  const handleBlockDates = useCallback(
    async (listingId: string, startDate: string, endDate: string, notes?: string) => {
      setIsLoading(true);
      try {
  ...
  const handleUnblockDates = useCallback(
    async (listingId: string, startDate: string, endDate: string) => {
      setIsLoading(true);
      try {
  ...
  const getAvailableDates = useCallback(
    async (listingId: string, startDate: string, nights: number) => {
      try {
  ...
  const setDefault = useCallback(
    async (listingId: string, startDate: string, endDate: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to set default availability';
        toast({
          title: 'Error',
          description: errorMessage,
          variant: 'destructive',
