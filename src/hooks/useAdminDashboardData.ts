  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        ...
        const total =
          revenueRes.data?.reduce((sum, payment) => sum + Number(payment.amount), 0) || 0;
        ...
        if (recentUsersCountRes.error || previousUsersCountRes.error) {
          ...
          setUserTrend(
            calculateTrend(recentUsersCountRes.count || 0, previousUsersCountRes.count || 0)
          );
        }
        ...
        if (recentPropertiesCountRes.error || previousPropertiesCountRes.error) {
          ...
          setPropertiesTrend(
            calculateTrend(
              recentPropertiesCountRes.count || 0,
              previousPropertiesCountRes.count || 0
            )
          );
        }
        ...
        if (recentRevenueRes.error || previousRevenueRes.error) {
          ...
          const recentRevenue =
            recentRevenueRes.data?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;
          const previousRevenue =
            previousRevenueRes.data?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;
        ...
        if (recentTicketsCountRes.error || previousTicketsCountRes.error) {
          ...
          const trend = calculateTrend(
            recentTicketsCountRes.count || 0,
            previousTicketsCountRes.count || 0
          );
        ...
        if (openTicketsRes.error) {
          ...
          const counts = (openTicketsRes.data || []).reduce(
            (acc, { category }) => {
              if (category) {
        ...
        if (pendingApplicationsRes.error) {
          console.error(
            'Error fetching pending rental applications:',
        ...
        const roleCounts = (userRolesRes.data || []).reduce(
          (acc: { [key: string]: number }, { role }) => {
        ...
        const distData = Object.entries(roleCounts).map(([name, value]) => ({
          name: name.charAt(0).toUpperCase() + name.slice(1),
          value,
          color: roleColors[name] || '#607D8B',
        }));
        setUserDistribution(distData);

        if (monthlyRevenueRes.error) throw monthlyRevenueRes.error;
        const revenueByMonth = (monthlyRevenueRes.data || []).reduce(
