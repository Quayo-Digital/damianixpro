...
export const usePaymentAccounting = () => {
    ...
    const [accounting, setAccounting] = useState<AccountingSummary>({
        totalRevenue: 0, platformFees: 0, agentCommissions: 0, ownerPayouts: 0, taxes: 0, pendingPayouts: 0
    });
    ...
    const loadPaymentsData = useCallback(async (filter: PaymentFilter) => {
        try {
            let query = supabase
                .from('rent_payments')
                .select(`id, payment_date, reference, amount, status,
                ...
                const ownerPayoutsWithNames: OwnerPayout[] = await Promise.all(
                    data.map(async (payout) => {
                        const { data: profileData, error: profileError } = await supabase
                            .from('profiles').select('full_name').eq('id', payout.owner_id).single();
                        return { ...payout, profiles: profileError ? { full_name: 'Unknown Owner' } : profileData };
                    })
                );
    ...
    const loadTabData = useCallback(async (tab: string, filter: PaymentFilter) => {
        try {
    ...
    return {
        activeTab, setActiveTab, dateRange, setDateRange, loading,
        filterProperty, setFilterProperty, properties, accounting,
        payments, ownerPayouts, processingPayout, selectedPayments,
        setSelectedPayments, loadData, handleProcessPayout, handleGenerateInvoice, handleSelectPayment
    };
}
