...
export const useEnhancedAgentData = () => {
  ...
  const [performanceMetrics, setPerformanceMetrics] = useState<AgentPerformanceMetrics | null>(
    null
  );
  ...
  const calculateStats = (
    ...
    const activeLeads = leadsData.filter((lead) =>
      ['new', 'contacted', 'qualified', 'viewing_scheduled', 'offer_made', 'negotiating'].includes(
        lead.status
      )
    ).length;
  ...
  const calculatePerformanceMetrics = (
    ...
    return {
      leadGeneration: {
        ...
        averageLeadValue:
          currentStats.totalLeads > 0
            ? leadsData.reduce((sum, lead) => sum + (lead.estimated_commission || 0), 0) /
      ...
      salesPerformance: {
        ...
        marketShare:
          currentStats.propertiesSold > 0
            ? Math.min(100, (currentStats.propertiesSold / 10) * 100)
            : 0,
      },
      commissionAnalytics: {
        ...
        topPerformingCategory:
          propertiesData.length > 0
            ? (() => {
                const counts = propertiesData.reduce(
                  (acc, prop) => {
                ...
                return (
                  Object.entries(counts).sort(([, a], [, b]) => b - a)[0]?.[0] || 'Residential'
                );
              })()
            : 'Residential',
      },
      clientMetrics: {
        ...
        clientRetentionRate:
          clientsData.length > 0
            ? Math.round(
                (clientsData.filter((c) => c.status === 'active').length / clientsData.length) * 100
              )
            : 0,
        ...
        referralRate:
          leadsData.length > 0
            ? Math.round(
                (leadsData.filter((l) => l.source === 'referral').length / leadsData.length) * 100
              )
            : 0,
      },
      marketIntelligence: {
        ...
        priceAccuracy:
          propertiesData.length > 0
            ? Math.round(
                (propertiesData.filter((p) => p.status === 'sold').length / propertiesData.length) *
                  100
              )
            : 0,
        marketTrends:
          propertiesData.length > 0
            ? [...new Set(propertiesData.map((p) => p.location))].map((location) => {
                ...
                const avgPrice =
                  locationProperties.reduce((sum, p) => sum + p.price, 0) /
                ...
                return {
                  ...
                  demandLevel:
                    soldCount > locationProperties.length * 0.7
                      ? ('high' as const)
                      : soldCount > locationProperties.length * 0.4
                        ? ('medium' as const)
                        : ('low' as const),
                };
              })
  ...
  const addNewLead = async (leadData: Partial<AgentLead>) => {
    try {
      if (user?.id) {
        await assertRoleScreeningForMonetization(user.id, 'agent');
      }
  ...
  useEffect(() => {
    const fetchAgentData = async () => {
      ...
      try {
        ...
        const realProperties: AgentProperty[] =
