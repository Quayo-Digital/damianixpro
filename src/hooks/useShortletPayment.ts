...
import { initializeBookingPayment, verifyBookingPayment, processRefund } from '@/services/shortlet/api/transactions';
import { getBookingById } from '@/services/shortlet/api/bookings';

export interface UseShortletPaymentReturn {
  initializePayment: (bookingId: string, amount: number, callbackUrl?: string) => Promise<{
    success: boolean;
    payment_url?: string;
    reference?: string;
    error?: string;
  ...
  processRefund: (bookingId: string, amount?: number, reason?: string) => Promise<{
    success: boolean;
    error?: string;
  }>;
  isLoading: boolean;
