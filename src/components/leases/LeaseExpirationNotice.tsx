...
export function LeaseExpirationNotice({ onRenew, onTerminate }: { 
  onRenew: () => void, 
  onTerminate: () => void 
}) {
  const { user } = useAuth();
  ...
  return (
    <Card className={daysRemaining <= 30 ? "border-red-200 bg-red-50 dark:bg-red-900/10" : "border-amber-200 bg-amber-50 dark:bg-amber-900/10"}>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge variant={daysRemaining <= 30 ? "destructive" : "outline"} className="flex gap-1 items-center">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span>
              ...
              <span className="font-semibold"> {format(parseISO(lease.end_date), 'MMM d, yyyy')}</span>
            </span>
          </div>
          
          <p className="text-sm">
            {daysRemaining <= 30 ? (
              <span className="font-medium">Your lease is expiring soon. Please choose whether you would like to renew or terminate your lease.</span>
            ) : (
              <span>Your lease will be expiring in {daysRemaining} days. Please consider your options below.</span>
            )}
          </p>
        </div>
      </CardContent>
