    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader className="sticky top-0 z-10 bg-background pb-4 pt-6">
          ...
          <DialogDescription>Detailed overview of your property cash flow</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 pb-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Card className="p-4">
              <h3 className="mb-2 text-sm font-medium text-muted-foreground">Total Cash In</h3>
              <p className="text-2xl font-bold">{formatAmount(totalInflow)}</p>
            </Card>
            <Card className="p-4">
              <h3 className="mb-2 text-sm font-medium text-muted-foreground">Total Cash Out</h3>
              <p className="text-2xl font-bold">{formatAmount(totalOutflow)}</p>
            </Card>
            <Card className="p-4">
              <h3 className="mb-2 text-sm font-medium text-muted-foreground">Net Cash Flow</h3>
              <p
          ...
          <div className="rounded-md border border-border bg-card p-4 text-card-foreground">
            <h2 className="mb-4 text-lg font-semibold">Cash Flow Trend</h2>
          ...
          <div className="rounded-md border border-border bg-card p-4 text-card-foreground">
            <h2 className="mb-4 text-lg font-semibold">Monthly Cash Flow</h2>
            <div className="h-[300px]">
              <ChartContainer config={chartConfig}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={actualData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <div className="rounded-md border border-border bg-card p-4 text-card-foreground">
            <h2 className="mb-4 text-lg font-semibold">Detailed Cash Flow</h2>
              ...
              <table className="w-full border-collapse">
                  ...
                  {cashFlowData.map((item, index) => {
                    ...
                    return (
                      <tr key={`${item.month}-${index}`} className="border-b">
                        ...
                        <td className="px-4 py-2 text-right">{formatAmount(item.inflow)}</td>
                        <td className="px-4 py-2 text-right">{formatAmount(item.outflow)}</td>
                ...
                <tfoot>
                  <tr className="border-t font-medium">
                    ...
                    <td className="px-4 py-2 text-right">{formatAmount(totalInflow)}</td>
                    <td className="px-4 py-2 text-right">{formatAmount(totalOutflow)}</td>
