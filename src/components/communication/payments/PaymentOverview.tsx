...
export const PaymentOverview = ({
  ...
  const activeRecurringPlans = payments.filter((p) => p.is_recurring && p.status === 'active');
    ...
    <Card>
      ...
      <CardContent>
        <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-lg bg-muted/50 p-4">
            <p className="mb-2 text-sm font-medium text-muted-foreground">Pending Payments</p>
            {isLoading ? (
              <Skeleton className="h-8 w-36" />
            ) : (
                ...
                >
                  <div>
                    <p className="font-medium capitalize">
                      {plan.category} - {plan.recurring_type}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Next payment: {formatAmount(plan.amount)} on{' '}
