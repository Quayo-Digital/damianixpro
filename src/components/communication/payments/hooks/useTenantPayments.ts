...
export const useTenantPayments = () => {
    ...
    const getTenantData = async () => {
      ...
      try {
        ...
        if (id) {
          ...
          setError(
            'No tenant profile found. Please contact your property manager to set up your tenant profile.'
          );
        }
      } catch (err) {
  ...
  const upcomingPayment = useMemo(() => {
    const futurePayments = payments
      .filter(
        (p) => (p.status === 'pending' || p.status === 'active') && (p.next_payment_date || p.date)
      )
      .filter((p) => {
