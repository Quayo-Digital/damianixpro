...
export const PaymentsTab = ({
  loading, payments, selectedPayments, processingPayout,
  onSelectPayment, onProcessPayout, onGenerateInvoice
}: PaymentsTabProps) => {
  ...
  const getOwnerGroups = (): Record<string, OwnerPaymentGroup> => {
    ...
    filteredPayments.filter(p => selectedPayments.includes(p.id)).forEach(payment => {
    ...
    <Card>
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={11} className="text-center py-8">Loading payments...</TableCell></TableRow>
            ) : filteredPayments.length > 0 ? (
              filteredPayments.map((payment) => (
                <TableRow key={payment.id}>
                  <TableCell>
                    <input type="checkbox" checked={selectedPayments.includes(payment.id)} onChange={() => onSelectPayment(payment.id)} className="h-4 w-4" />
                  </TableCell>
                  <TableCell>{new Date(payment.payment_date).toLocaleDateString()}</TableCell>
                  <TableCell>{payment.reference}</TableCell>
                  <TableCell>{payment.property_tenants?.properties?.name || 'Unknown'}</TableCell>
                  <TableCell>{payment.property_tenants?.tenants ? `${payment.property_tenants.tenants.first_name} ${payment.property_tenants.tenants.last_name}` : 'Unknown'}</TableCell>
                  <TableCell className="text-right">₦{payment.amount.toLocaleString()}</TableCell>
                  <TableCell className="text-right">₦{(payment.breakdown?.platform_fee || 0).toLocaleString()}</TableCell>
                  <TableCell className="text-right">₦{(payment.breakdown?.agent_commission || 0).toLocaleString()}</TableCell>
                  <TableCell className="text-right font-medium">₦{(payment.breakdown?.owner_amount || 0).toLocaleString()}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => onGenerateInvoice(payment.id)}>
              ...
              <TableRow><TableCell colSpan={11} className="text-center py-8">No pending payments found</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </CardContent>
  ...
  const renderMobileView = () => (
    <div className="space-y-4">
      {loading ? (
        ...
        <p className="text-center py-8 text-muted-foreground">No pending payments found</p>
      )}
    </div>
  );
          ...
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{selectedPayments.length} payments selected</p>
              </div>
              <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2">
                <span className="text-sm font-medium">Total: ₦{totalSelectedAmount.toLocaleString()}</span>
                {Object.values(ownerGroups).map(group => (
                  <Button
                    key={group.owner.id} variant="default" size="sm" disabled={processingPayout}
                    onClick={() => onProcessPayout(group.owner.id, group.total, group.payments.map(p => p.id))}
                  >
