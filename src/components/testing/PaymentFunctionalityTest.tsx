  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      try {
        ...
        if (service && typeof service.initializeFlutterwavePayment === 'function') {
          updateTestResult(
            'Payment Service Initialization',
            'passed',
          ...
          updateTestResult(
            'Payment Service Initialization',
            'failed',
            'Payment service not properly initialized'
          );
        }
      } catch (error) {
        updateTestResult(
          'Payment Service Initialization',
          'failed',
          `Service initialization error: ${error}`
        );
      }
      ...
      try {
        if (processPaymentWithMethod && verifyPayment && loadPaymentData) {
          updateTestResult(
            'Payment Hook Integration',
            'passed',
            'All payment hook methods available'
          );
        } else {
      ...
      // Test 3: Flutterwave Payment Processing (Mock)
      ...
      try {
        if (typeof paymentService.initializeFlutterwavePayment === 'function') {
          updateTestResult(
            'Flutterwave Payment Processing',
            'passed',
            'Flutterwave integration ready (mock test)'
          );
        } else {
          updateTestResult(
            'Flutterwave Payment Processing',
            'failed',
            'Flutterwave method not available'
          );
        }
      } catch (error) {
      ...
      // Test 4: Bank Transfer Processing
      ...
      try {
        if (typeof paymentService.initializeBankTransferPayment === 'function') {
          updateTestResult(
            'Bank Transfer Processing',
            'passed',
            'Bank transfer processing available'
          );
        } else {
          updateTestResult(
            'Bank Transfer Processing',
            'failed',
            'Bank transfer method not available'
          );
        }
      } catch (error) {
      ...
      // Test 5: USSD Payment Processing
      ...
      try {
        if (typeof paymentService.initializeUSSDPayment === 'function') {
          updateTestResult(
            'USSD Payment Processing',
            'passed',
            'USSD payment processing available'
          );
        } else {
      ...
      // Test 7: Payment History Loading
      ...
      try {
        ...
        updateTestResult(
          'Payment History Loading',
          'passed',
          `Payment data loaded. History: ${paymentHistory.length} items, Pending: ${pendingPayments.length} items`
        );
      } catch (error) {
        updateTestResult('Payment History Loading', 'failed', `Data loading error: ${error}`);
      }

      // Test 8: Payment Interface Component
      ...
      try {
        ...
        updateTestResult(
          'Payment Interface Component',
          'passed',
          'Payment interface component available'
        );
      } catch (error) {
      ...
      try {
        if (
          typeof paymentService.getPaymentHistory === 'function' &&
          typeof paymentService.getPendingPayments === 'function'
        ) {
          updateTestResult(
            'Database Integration',
            'passed',
            'Database integration methods available'
          );
        } else {
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          {totalTests > 0 && (
            <div className="flex items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">{totalTests} Total Tests</span>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {testResults.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Detailed results for each payment functionality test</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <Alert className="mb-4">
              ...
              <AlertDescription>
                This is a live payment interface. For testing, use small amounts and test payment
                methods only.
              </AlertDescription>
            </Alert>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Overview of all available payment processing options</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="rounded-lg border p-4">
              ...
              <p className="text-sm text-muted-foreground">
                Card payments, bank transfers, and USSD via Flutterwave
              </p>
              <div className="mt-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Current status of payment processing components</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
