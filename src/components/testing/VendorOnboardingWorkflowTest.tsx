...
const VENDOR_TEST_DATA: VendorTestData = {
  ...
  hourlyRate: 5000,
  description:
    'Professional maintenance services with 8+ years of experience. Specializing in plumbing, electrical, and general property maintenance.',
...
export function VendorOnboardingWorkflowTest() {
  ...
  const updateStepStatus = (
    ...
    setTestResults((prev) =>
      prev.map((result) => (result.id === stepId ? { ...result, status, data, error } : result))
    );
  };
  ...
  const runComprehensiveTest = async () => {
    ...
    setTestResults((prev) =>
      prev.map((result) => ({ ...result, status: 'pending', data: undefined, error: undefined }))
    );
    ...
    try {
      ...
      if (retrieveError || !retrievedVendor) {
        updateStepStatus(
          ...
          'error',
          null,
          retrieveError?.message || 'Vendor not found'
        );
      } else {
        const dataMatches =
            ...
            <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  ₦{VENDOR_TEST_DATA.hourlyRate.toLocaleString()}
                </p>
              </div>
            </div>
            <div>
