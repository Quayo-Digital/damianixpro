...
import { CheckCircle, XCircle, Clock, User, Building, Mail, Phone, MapPin, Star, AlertTriangle } from 'lucide-react';
...
export const ComprehensiveAgentWorkflowTest: React.FC = () => {
  ...
  const initializeTestSteps = (): TestStep[] => [
    ...
    },
    {
      ...
      description: 'Verify that SmartRoleSelection suggests "agent" role based on email and company',
      status: 'pending'
    },
    {
      id: 'create-user',
  ...
  const runTest = async () => {
    ...
    try {
      ...
      const { AutomaticRoleAssignmentService } = await import('@/services/roleAssignment/automaticRoleAssignment');
      ...
      if (roleResult.suggestedRole === 'agent' && roleResult.confidence > 70) {
        ...
        updateStepStatus('test-role-suggestion', 'error', roleResult, 'Role assignment did not suggest agent role with high confidence');
      }
      ...
      if (roleError) {
        ...
        updateStepStatus('verify-role-assignment', 'error', roleData, `Expected 'agent' role, got '${roleData.role}'`);
      }
      ...
      try {
        ...
        if (profileUpdateError) {
          updateStepStatus('test-agent-onboarding', 'error', null, `Profile update error: ${profileUpdateError.message}`);
        } else {
          updateStepStatus('test-agent-onboarding', 'success', { message: 'Profile updated with available agent data' });
        }
      } catch (error) {
        updateStepStatus('test-agent-onboarding', 'error', null, `Agent onboarding test failed: ${error}`);
      }
      ...
      try {
        ...
        updateStepStatus('verify-profiles-data', 'error', null, `Profile verification failed: ${error}`);
      }
      ...
      try {
        ...
        if (agentError) {
          if (agentError.message.includes('relation "agents" does not exist')) {
            updateStepStatus('verify-agents-table', 'warning', null, 'Agents table does not exist yet (migration needed)');
          } else {
        ...
        updateStepStatus('verify-agents-table', 'error', null, `Agents table verification failed: ${error}`);
      }
      ...
      updateStepStatus('test-agent-dashboard', 'success', { message: 'Agent dashboard access verified (simulated)' });
      ...
      try {
        ...
        if (propertiesError) {
          ...
          if (propertiesError.message.includes('column') && propertiesError.message.includes('does not exist')) {
            ...
            if (fallbackError) {
              updateStepStatus('test-property-visibility', 'error', null, `Properties table access failed: ${fallbackError.message}`);
            } else {
              updateStepStatus('test-property-visibility', 'warning', { 
                message: 'Property visibility test completed with limited columns (schema migration needed)',
        ...
        updateStepStatus('test-property-visibility', 'error', null, `Property visibility test failed: ${error}`);
      }
      ...
      try {
        ...
        updateStepStatus('cleanup', 'warning', { message: `Cleanup completed with some issues: ${error}` });
      }
      ...
      updateStepStatus(testResults[currentStep - 1]?.id || 'unknown', 'error', null, error instanceof Error ? error.message : 'Unknown error');
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            End-to-end test of agent registration, intelligent role assignment, onboarding, and dashboard access.
            Tests WF-002 (Agent Onboarding) and WF-003 (Automated Role Assignment).
          </CardDescription>
        </CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-gray-50 rounded-lg">
                <div className="text-2xl font-bold text-gray-600">{testResults.length - successCount - errorCount}</div>
            ...
            {successCount === testResults.length && (
              <Alert className="mt-4 border-green-200 bg-green-50">
                ...
                <AlertDescription className="text-green-800">
                  <strong>All tests passed!</strong> The comprehensive agent workflow is working correctly.
                  Both WF-002 (Agent Onboarding) and WF-003 (Automated Role Assignment) are functioning as expected.
                </AlertDescription>
              </Alert>
            )}
            ...
            {errorCount > 0 && (
              <Alert className="mt-4 border-red-200 bg-red-50">
                ...
                <AlertDescription className="text-red-800">
                  <strong>Some tests failed.</strong> Please review the error details above and address any issues.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
