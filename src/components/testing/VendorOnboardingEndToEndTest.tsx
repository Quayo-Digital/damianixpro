...
export const VendorOnboardingEndToEndTest: React.FC = () => {
  ...
  const testDatabaseSchema = async () => {
    ...
    await runTest(0, 1, async () => {
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          throw new Error(
            'Vendor_jobs table does not exist - run vendor onboarding migration first'
          );
        }
  ...
  const testJobManagement = async (passedVendorProfile?: any): Promise<any> => {
    ...
    if (!vendorProfile) {
      ...
      throw new Error(
        'No vendor profile for job tests - ensure user and profile creation tests passed first'
      );
    }
    ...
    const vendorJob = await runTest(2, 0, async () => {
      ...
      const { data, error } = await supabase.from('vendor_jobs').insert(jobData).select().single();
  ...
  const testIntegration = async (passedVendorProfile?: any, passedVendorJob?: any) => {
    ...
    if (!vendorProfile) {
      ...
      console.error('Full testData:', JSON.stringify(testData, null, 2));
      throw new Error(
        'No vendor profile for integration tests - ensure profile creation tests passed'
      );
    }
    ...
    console.log(
      'Starting integration tests with profile:',
      vendorProfile.id,
      'and job:',
      vendorJob.id
    );
  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      if (typeof createdData !== 'object') {
        console.error(
          'testUserAndProfileCreation returned non-object:',
          typeof createdData,
          createdData
        );
      ...
      if (!createdData.vendorProfile) {
        ...
        throw new Error(
          'User creation completed but vendor profile not available in returned data'
        );
      }

      console.log(
        'User and profile creation tests completed - vendor profile available:',
        createdData.vendorProfile.id
      );
  ...
  const completedTests = testSuites.reduce(
    (sum, suite) =>
      sum + suite.tests.filter((t) => t.status === 'passed' || t.status === 'failed').length,
    0
  );
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-2">
                <div className="flex items-center gap-4">
                  ...
                  <Button onClick={cleanup} variant="outline" disabled={isRunning}>
                    Cleanup Test Data
                  </Button>
                </div>
