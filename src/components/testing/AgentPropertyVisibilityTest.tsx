  ...
  const runSecurityTests = async () => {
    ...
    try {
      ...
      results.push({
        ...
        message: hasSecurityLogging ? '✅ Security logging active' : '⚠️ Security logging may be missing',
      ...
      results.push({
        ...
        message: hasUnauthorizedData ? '🚨 Potential data leak detected!' : '✅ No unauthorized data access',
        details: hasUnauthorizedData ? 'User has access to properties they should not see' : 'All property access is authorized'
      });
      ...
      results.push({
        ...
        message: userRoleRecord ? '✅ User role properly assigned' : '🚨 User role not found in database',
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Comprehensive test to verify that the Agent Property Visibility security fix is working correctly.
            This test ensures agents only see assigned properties and other roles have appropriate access.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              <strong> Properties Visible:</strong> {properties.length} | 
              <strong> Total Properties:</strong> {allProperties?.length || 'Loading...'}
            </AlertDescription>
          </Alert>
          ...
          {testResults.length > 0 && (
            <Alert className={failedTests > 0 ? 'border-red-500 bg-red-50' : 'border-green-500 bg-green-50'}>
              <Shield className="h-4 w-4" />
              <AlertDescription>
                <strong>Security Status:</strong> {failedTests === 0 ? 
