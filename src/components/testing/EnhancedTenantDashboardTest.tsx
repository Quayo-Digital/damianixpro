...
export function EnhancedTenantDashboardTest() {
  ...
  const resetTests = () => {
    setTestResults(prev => 
      prev.map(category => ({
        ...category,
        tests: category.tests.map(test => ({ ...test, status: 'pending', duration: undefined, error: undefined }))
      }))
    );
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-green-50 to-blue-50 border-green-200">
        <CardHeader>
          ...
          <CardDescription>
            Comprehensive testing for the Enhanced Tenant Dashboard functionality, UI components, and business logic
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="flex justify-between text-sm text-gray-600">
                  <span>
                    {category.tests.filter(t => t.status === 'passed').length} of {category.tests.length} tests passed
                  </span>
                  <span>
                    Success Rate: {category.tests.length > 0 ? 
                      ((category.tests.filter(t => t.status === 'passed').length / category.tests.length) * 100).toFixed(1) : 0}%
                  </span>
                </div>
              </CardContent>
            </Card>
