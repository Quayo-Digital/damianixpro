-- Setup Missing Storage Buckets (Fixed Version)
...
BEGIN
  ...
  -- Drop existing policies for application-documents bucket
  ...
  -- Drop existing policies for property-documents bucket
  ...
  DROP POLICY IF EXISTS "Users can update their own property documents" ON storage.objects;
...
-- 7. Create RLS policies for application-documents bucket
...
-- 8. Create RLS policies for property-documents bucket
...
-- 9. Create RLS policies for property-images bucket
...
-- 10. Create documents table for tracking uploaded documents
...
-- Drop existing policies on documents table if they exist
